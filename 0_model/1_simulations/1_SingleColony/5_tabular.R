...
# select scenario
sc.lab = as.vector(subset(scens.id, scenario %in% c("tab"))) ##change scenarios here
id = sc.lab$id
...
# make new folder with name giving parameters for automatically saving outputs ----
sim.wd = paste(work.dir,"1_Simulations", "1_Outputs", "1_SingleColony", sep="/")
setwd(sim.wd)
...
for (run in 1:runs){ # multiple simulation runs
  ...
  ftypes = data.frame(names = c("encrusting", "flexihem", "digitate", "corymbose", "tabular",
                                "mushroom", "fingers", "cone", "hedgehog", "staghorn"))   #new morphologies
  ...
  for (ts in 1:timesteps){
    ...
    if (nrow(colonymap) > 0) { # only run if there are at least 1 colony in the world
	  ...
	  if (colonymap$res[ii] > res.cap & colonymap$ft[ii]==5) colonymap$res[ii] = 8*res.cap  # cap resources
}
