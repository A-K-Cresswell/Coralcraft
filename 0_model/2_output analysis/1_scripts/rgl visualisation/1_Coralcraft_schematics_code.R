...
sch = paste(work.dir,"1_Simulations", "2_Scenarios", "1_schematics", sep="/")
setwd(work.dir)
...
uM2 <- read.csv("uM.csv") #rgl dimensions
uM3 <- data.matrix(uM2)
...
for (run in 1:runs){ # multiple simulation runs
  ...
  for (ts in 1:timesteps){
    ...
    if (nrow(colonymap) > 0) { # only run if there are at least 1 colony in the world
      for (i in colonymap$colonyid) { # take each colony separately
        ...
        if (colonymap$res[ii] > res.cap & colonymap$ft[ii]!=5) colonymap$res[ii] = res.cap  # cap resources
        if (colonymap$res[ii] > res.cap & colonymap$ft[ii]==5) colonymap$res[ii] = 2*res.cap  # cap resources for tabulars
 ...
 # high intensity disturbances are set to occur at halfway intervals between the low intensity for fixed case
  ...
  if (draw == 1 & run == 1) { # if you want to plot in 3D
    ...
    spheres3d(loc[,1],loc[,2],loc[,3],color=colpal3[ftss], alpha=5)
    shade3d(qmesh3d(verticesfloor, indices),color='lightgrey')
    shade3d(qmesh3d(verticesback, indices),color='white')
    shade3d(qmesh3d(verticesside, indices),color='white')
    text3d(-6,-6,0,paste('Time step', ts))
    if (save3D == 1) {
      # setwd(sch) #for schematics
