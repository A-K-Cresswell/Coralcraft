...
## mean coverage from top view
## mean sheltered cells from z=1
## mean sheltered cells from z=5
## mean sheltered cells from z=10


#### DAPHNE TO UPDATE SCRIPT
## for each dataframe
# simulation dates 
# ts in for loops
...
# saving outputs
setwd(output)
write.csv(df_metrics, file = "df_metrics_sheltop_4_13.csv")
################################################################
## binding data frames ##

# only need to do for 1fts (FR scenarios not compatible)
setwd(output)
df_metrics = read.csv("df_metrics_cover.csv") #all.10, bottom 5, complex 5
df_1fts = read.csv("df_metrics_cover_1fts.csv") # 1fts only

df2 <- df_metrics %>% 
  filter(scenarios == "All 10")

df_metrics <- rbind(df2, df_1fts)
write.csv(df_metrics, file = "df_metrics_cover_1fts_all.10.csv")
