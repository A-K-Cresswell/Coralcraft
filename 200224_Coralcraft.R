...
# Appendix C of Cresswell et al. 2020, "Frequent hydrodynamic disturbances decrease the morphological diversity and structural complexity of simulated coral communities"

# Set your working directory to a folder containing this script and Coralcraft_morphology_design_code.R
...
source('Coralcraft_morphology_design_code.R') ## need to run this once to load the growth forms
load(file="ftcelllist") # load growth forms



# make new folder with name giving parameters for automatically saving images and outputs ----
...
# plotting parameters ----
draw = 0 # if set to 1, will plot in 3D each timestep
save3D = 0 # if set to 1, will save 3D plot in each timestep
drawscatter = 0 # will plot and save a scatterplot
...
# # below shades the sides of the world
indices <- c( 1, 2, 3, 4 )
...
for (run in 1:runs){ # multiple simulation runs
  ...
  light = array(rep(light.level * light.atten^((ws-1):0), each=ws*ws), dim=c(ws,ws,ws)) # this instantly sets up initial light through the world #fix here
  ...
  for (i in 1:n.initial.colonies) world[rows[i], cols[i], 1] = i # give colony id to location in world
  
  
  ftypessaveall = NULL # dataframe for saving the number of cells, percentage cover, number of colonies (and rugosity?) in each timestep
  ...
  #ftypes = data.frame(names = paste('type',1:nfts))  ## give these better names?? 
  ...
  ftsinc_list = c("encrusting", "hemispherical", "tabular", "branching", "corymbose") # select which functional types to investigate
  ...
  for (ts in 1:timesteps){
    ...
    if (nrow(colonymap) > 0) { # only do stuff if there are actually colonies
      for (i in colonymap$colonyid) { # Take each colony separately
        ii = which(colonymap$colonyid == i)
        colonymap$res[ii] = colonymap$res[ii] + sum(lightuptake[world == i & !dead] - maint) # update amount of resources  (AKC 190201 - I removed dependence on * light.to.resources)
      }
      ## cap resources
      colonymap$res[colonymap$res>res.cap] = res.cap
      
      ### Death ---- 
      dead[lightuptake < maint & world > 0] = TRUE # dies if not getting enough light to maintain itself 
      ...
      if (ncolonies > 0) {
        ...
        for (i in colonygrowthorder) {
          ...
          ngrowth = floor(thiscolony$res / ftsinc$resource.to.growth[which(ftsinc$ftnum == thiscolony$ft)]) #AKCchange181112
          ...
          if (ngrowth > 0) {
            ...
            priorities = ftcelllist[[thiscolony$ft]][neighbsinlist] ##issue here?
    ...
    for (i in 1:nfts) {
      ...
      if (nrow(t1) > 0) { 
        ftypessave$cells[i] = sum(world %in% t1$colonyid) # want to see where world is the colony id not the functional type #includes dead
    ...
    totalSA=sum(ifempty)
    #print(totalSA)
    #print(nrow(currentcells))
    # here add on empty z = 1 cells
    emptyz1 = sum(world[,,1]==0)
    totalSA = totalSA + emptyz1
    ...
    ftypessave$no.colonies[which(ftypessave$c.1.nfts...dead.. =="dead")] = sum(colonymap$stillalive == F) # i think dead colonies get removed anyway though
    ...
    if ((ts+spawn.freq/2+6)%%spawn.freq == 0) { #spawing offset to occur in half years + 6 timesteps
      print("SPAWNING")
      #posslocs = which(world[,,1]==0)						## 
      reclocations = sample(1:(ws*ws) , nnewrecruits) # x y locations for new recruit
        ...
        if (world[rows[i], cols[i], 1] == 0) { # if there isn't already a coral there
          ...
          if (randomrecruits == 1) {
            ...
            if (length(ftsinc$ftnum) > 1) { #else sample from the available fts
              colonymapnew = data.frame(colonyid = newcolID ,
                                        ft = sample(ftsinc$ftnum, 1), # had to add the over if() for when there is only one colony type
                                        age = 1,
                                        res = start.res)} # resources
          } else {
            if (length(ftsinc$ftnum) == 1) { # if there is only one ft choose that one
          ...
          #colonymap$ft = as.numeric(colonymap$ft)
        }
      }
    ...
    if (nrow(colonymap) > 0) {
      ...
      colonymap = subset(colonymap, !whichremove ) #last bit removed
      # set world to zero and dead to FALSE for these new cells
      # world contains to colony id. If colonyid is in colonyremove world should be set to zero
  ...
  if ((randomdist == "fixed" & ts%%freq.low == 0 & nrow(colonymap) >0 ) | (randomdist != "fixed" & runif(1) < (1/freq.low) & nrow(colonymap) >0) ) { 
 ...
 # jittered to occur at halfway intervals between the low intensity  for fixed
  ...
  if (draw == 1 & run == 1) { # if you want to plot in 3D
    ...
    spheres3d(loc[,1],loc[,2],loc[,3],color=colpal[ftss], alpha=5)#,alpha=0.9,coralpolyps$sz*0.7)#+0.3*runif( dim(coralpolyps)[1] )) ## last bit makes circles random sizes
    shade3d(qmesh3d(verticesfloor, indices),color='black')
    shade3d(qmesh3d(verticesback, indices),color='darkgrey')
    shade3d(qmesh3d(verticesside, indices),color='grey')
  ...
  if (drawscatter == 1 & run == 1) { # if you want to plot in 3D
    loc = which(world > 0,arr.ind = TRUE)
    ids = world[which(world>0)]
    ifdead = dead[which(world>0)]
    ftss = unlist(sapply(ids , function(thisid) {
